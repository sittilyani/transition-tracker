...
$training_types = mysqli_query($conn, "SELECT * FROM training_types ORDER BY trainingtype_name");
...
if (isset($_POST['action'])) {
    ...
    if (empty($selected_staff)) {
        ...
        try {
            ...
            if ($session_id > 0 && $session_data['status'] == 'draft') {
                ...
                mysqli_query($conn, "DELETE FROM training_participants WHERE session_id = $session_id");
                ...
                $session_sql = "INSERT INTO training_sessions (
                    session_code, course_id, duration_id, training_type_id, location_id,
            ...
            foreach ($selected_staff as $staff_id) {
                ...
                $participant_sql = "INSERT INTO training_participants (session_id, staff_id)
                                   VALUES ($session_id, $staff_id)";
                mysqli_query($conn, $participant_sql);
            }

            // If submitted, update submitted info (for new sessions)
            if ($action == 'submit' && !($session_id > 0 && $session_data['status'] == 'draft')) {
...
// Fetch staff list for selection (excluding photo, fingerprint to reduce data load)
    ...
    <div class="container">
        ...
        <form method="POST" id="trainingForm">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="training_type_id" required>
                            ...
                            $selected_type = $session_data['training_type_id'] ?? ($draft_data['training_type_id'] ?? '');
                            while ($row = mysqli_fetch_assoc($training_types)):
                            ?>
            ...
            <div class="button-group">
                ...
                <?php if ($session_id > 0 && $session_data['status'] == 'draft'): ?>
