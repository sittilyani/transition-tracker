...
// Date range filter — no table alias, works in both union branches
...
// Each UNION branch needs its own copy of $params — multiply accordingly
$union_params       = !empty($params) ? array_merge(...array_fill(0, $union_branch_count, $params)) : [];
$union_types        = str_repeat($types, $union_branch_count);

// Stats query uses exactly 2 branches always, so always needs params x2
$stats_branch_count = 2;
$stats_params       = !empty($params) ? array_merge(...array_fill(0, $stats_branch_count, $params)) : [];
$stats_types        = str_repeat($types, $stats_branch_count);

// Get total count for pagination
$count_query = "SELECT COUNT(*) as total FROM ($combined_query) as combined";
$count_stmt = $conn->prepare($count_query);
if (!empty($union_params)) {
    $count_stmt->bind_param($union_types, ...$union_params);
}
$count_stmt->execute();
...
if (!empty($union_params)) {
    $stmt->bind_param($union_types . "ii", ...array_merge($union_params, [$limit, $offset]));
} else {
